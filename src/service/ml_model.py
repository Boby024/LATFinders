...
def accurate_model(indexedDataset):
    ...
    pred['forecast1'] = result_1.predict(start=round(len(indexedDataset)*0.8),end=len(indexedDataset)-1,dynamic=True)
    
    #Model 2:
    result_2 = SARIMA_model(indexedDataset,(2,0,0),(2,1,0,12))
    pred['forecast2'] = result_2.predict(start=round(len(indexedDataset)*0.8),end=len(indexedDataset)-1,dynamic=True)
    
    #Model 3:
    result_3 = SARIMA_model(indexedDataset,(1,0,1),(2,1,0,12))
    pred['forecast3'] = result_3.predict(start=round(len(indexedDataset)*0.8),end=len(indexedDataset)-1,dynamic=True)
    
    #Model 4:
    result_4 = SARIMA_model(indexedDataset,(2,0,1),(0,1,0,12))
    pred['forecast4'] = result_4.predict(start=round(len(indexedDataset)*0.8),end=len(indexedDataset)-1,dynamic=True)
    
    #Model 5:
    result_5 = SARIMA_model(indexedDataset,(2,0,2),(2,1,0,12))
    pred['forecast5'] = result_5.predict(start=round(len(indexedDataset)*0.8),end=len(indexedDataset)-1,dynamic=True)
    
    #Model 6:
    result_6 = SARIMA_model(indexedDataset,(3,0,1),(0,1,0,12))
    pred['forecast6'] = result_6.predict(start=round(len(indexedDataset)*0.8),end=len(indexedDataset)-1,dynamic=True)
    ...
    rmse_val = [sqrt(mean_squared_error(pred['overall_rating'], pred['forecast1'])),
                sqrt(mean_squared_error(pred['overall_rating'], pred['forecast2'])),
                sqrt(mean_squared_error(pred['overall_rating'], pred['forecast3'])),
                sqrt(mean_squared_error(pred['overall_rating'], pred['forecast4'])),
                sqrt(mean_squared_error(pred['overall_rating'], pred['forecast5'])),
    ...
    indexedDataset['forecast'] = best_Model.predict(start=0,end=len(indexedDataset)-1,dynamic=True)
...
def future_trend(indexedDataset,result, months_num):
    ...
    future_dates=[indexedDataset.index[-1]+ DateOffset(months=x)for x in range(0,months_num)]
    future_datest_df=pd.DataFrame(index=future_dates[1:],columns=indexedDataset.columns)
                    
    future_df=pd.concat([indexedDataset,future_datest_df])
    future_df['forecast'] = result.predict(start=len(indexedDataset)-1,end=len(indexedDataset)+ months_num-1,dynamic=True)
        ...
        if np.isnan(future_df.iat[i,7]) == True:
            for j in range(1, len(future_df)-i):
                if(np.isnan(future_df.iat[i+j,7]) == False):
                    future_df.iat[i,7] = (future_df.iat[i-1,7] + future_df.iat[i+j,7])/2
                    break
...
def final(result):
    ...
    graphs["future_trend_fig"] = plot_future_trend(future_df, uni_name, course_name)

    graphs["current_trend_mean"] = round(indexedDataset['overall_rating'].mean(), 2)
